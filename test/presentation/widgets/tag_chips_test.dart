...
void main() {
  testWidgets('renders one chip per tag with its stored color',
      (tester) async {
    await tester.pumpWidget(wrap(
      TagChips(tags: [
        tag(1, 'urgent', '#FFD3D3'),
        tag(2, 'normal flow', '#CFEEFF'),
      ]),
    ));
  ...
  testWidgets('invalid stored color falls back to the theme tag color',
      (tester) async {
    await tester.pumpWidget(wrap(
      TagChips(tags: [tag(1, 'legacy', 'tag-yellow')]),
    ));
  ...
  testWidgets('remove button reports the tapped tag', (tester) async {
    ...
    await tester.pumpWidget(wrap(
      TagChips(tags: tags, onRemove: (t) => removed = t),
    ));
